...
export default async function handler(req, res) {
  ...
  const { geminiAnalysis, clientId, prompt } = req.body;
  const GAS_API_URL       = process.env.GAS_API_URL;
  const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY;

  if (!geminiAnalysis) {
    return res.status(400).json({ error: 'geminiAnalysis が必要です' });
  }
...
=== Geminiによる動画分析結果（OCR・テキスト書き起こし含む）===
${geminiAnalysis}
=== 分析終わり ===

## 厳守ルール
- Geminiの分析結果にテキスト情報が含まれている場合、それを必ず参照してアドバイスすること
- 「テキストが確認できない」「文字が見えない」「オーバーレイは確認できない」などと言ってはいけない。Geminiがすでに書き起こしているので、その内容を使うこと
- 動画を「直接見ていない」ことは言及不要。Geminiの分析結果がすべての情報源であると理解した上で回答する
